...
$question_id=$_GET['id'];
//look at url, find out the value of the var id
$sql_get_question = "SELECT * FROM questions_tbl WHERE question_id=$question_id";
//give me all the col, but only for the q id that matches this one
$result = $makeconnection->query($sql_get_question);//press send! put answer into $result
$row = $result->fetch_assoc();//convert result to array
if (isset($_POST['submit'])) {//if form was submitted... time to process.
	//print_r($row)
//echo "yes";
...
$replace_str = '"';// what is a problem char? the double quote
$question_statement = str_replace($replace_str, '&quot;', $question_statement); // use the html character for double quote
					?>
					<td width="40%" height="150" align="right" valign="top">Statement</td>
<td valign="top"><input type="text" name="question_statement" id="question_statement" required size="50" maxlength="150" value="<?php echo $question_statement ?>">
					</td>
				</tr>
