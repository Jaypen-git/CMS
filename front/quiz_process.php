...
<body>
	...
	///convert percent to letter

	if ($student_percent >=90){//between 90 and 100

	$student_lettergrade="A";

	}elseif($student_percent >=80 && $student_percent <=89 ){//between 80 and 89

	$student_lettergrade="B";

	}elseif($student_percent >=70 && $student_percent <=79 ){//between 70 and 99

	$student_lettergrade="C";

	}elseif($student_percent >=60 && $student_percent <=69 ){//between 60 and 69
	...
	}//end of else if, now we got a lettergrade!
		
	echo "<h2>lettergrade: $student_lettergrade</h2>";
		
$sql_insert_result="INSERT INTO students_tbl
(student_first, student_last, student_email, student_points, student_outof, student_percent, student_datetime, student_lettergrade)
VALUES
('$student_first', '$student_last', '$student_email', $points_earned, $max_avail_points, $student_percent, '$student_datetime', '$student_lettergrade')";
...
if ($result = $makeconnection->query($sql_insert_result )){
	...
	echo "error";
			...
			<br>
	<a href="../index.php"><button>developer: Back to top portal</button></a>	
